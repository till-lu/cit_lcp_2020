...
## for testing
testing = False
# True for testing, False for real recording
###
main_ddline = 1 # sec
...
def ending():
    ...
    info = 'Danke für die Teilnahme. Wenn Sie möchten, können Sie gehen, aber bitte seien Sie leise dabei.\n\nKurze Information über den Test:\n\nIn dieser Studie versuchen wir, die Details aus der zu Beginn verfassten E-Mail von anderen Items zu unterscheiden. Ziel des Tests ist es, anhand von Reaktionszeiten herauszufinden, wenn eine Person versucht, Wissen über bestimmte Items zu verschleiern bzw. zu verheimlichen. Dies geschieht auf Basis der Vermutung, dass Reaktionszeiten für die Items, die für die anfängliche E-Mail benötigt wurden langsamer ausfallen als für andere Items. Hauptanliegen dieser Studie ist herauszufinden, ob dies auch funktioniert, wenn die Funktionsweise des Tests vorher offen gelegt wurde.\n\nFür weitere Informationen wenden Sie sich bitte an den Versuchsleiter (oder schreiben Sie eine E-Mail an Gaspar Lukacs (gaspar.lukacs@univie.ac.at)).'
...
def target_check_one():
    ...
    if condition in [1,2,5,6]:
        ...
        if rounds == 1:
            ...
            show_instruction('Es folgt die letzte Runde des Abfragetests. Lernen Sie die unten stehenden Wortpaare gründlich auswendig, drücken Sie die Leertaste, um fortzufahren. \n\n' + target_guilty_list)
    else:
...
def next_block():
    ...
    if block_num == 1:
        ...
        target_check_one()
        target_check_one()
        show_instruction('Nun folgt der Test zu der Liste mit Wortkombinationen, die Sie gerade gelernt haben. Sie werden verschiedene Wortkombinationen sehen, die nacheinander auf dem Monitor gezeigt werden. Bei jeder dieser Kombinationen sollen Sie so schnell wie möglich angeben, ob diese in der eben gelernten Liste enthalten war oder nicht. War die Wortkombination in der zuvor gelernten Liste enthalten, drücken Sie „J“ (JA). Wird eine andere Wortkombination gezeigt, so drücken Sie „F“ (NEIN). Verwenden Sie dazu Ihre beiden Zeigefinger und positionieren sie direkt über den Tasten, damit Sie so schnell und akkurat wie möglich reagieren können. Leertaste drücken, um fortzufahren.')
